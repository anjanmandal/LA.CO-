...
export async function getOverview({ from=2015, to=2025 } = {}) {
  const r = await fetch(`${BASE}/metrics/overview?from=${from}&to=${to}`);
...
export async function getFacilityTrend(id) {
  const r = await fetch(`${BASE}/metrics/facility/${id}/trend`);
  const d = await r.json();
...
export async function getMethods(qs) {
  const r = await fetch(`${BASE}/metrics/methods${qs?`?${qs}`:''}`);
  const d = await r.json();
...
export async function getReconciliation(facilityId) {
  const r = await fetch(`${BASE}/metrics/reconciliation/${facilityId}`);
  const d = await r.json();
...
export async function getReconciliationExplain(facilityId, year) {
  ...
  const r = await fetch(`${BASE}/metrics/recon/explain/${facilityId}?${params.toString()}`);
  console.log('RECON EXPLAIN', r);
  const d = await r.json();
...
export async function getAnomalies({ facilityId, sector, z=3.5, source='reported' }) {
  ...
  const r = await fetch(`${BASE}/analytics/anomalies?${params.toString()}`);
  console.log('ANOMALIES', r);
  const d = await r.json();
